    ...
    def generate_core_deep_fields(
        ...
        prompt = (
            ...
            "B. items：对每篇文章输出三条线索（全中文、信息密度高，每条 2~3 句）：\n"
                "   1) method_point（120~200 字）：这项工作具体做了什么——研究对象、核心方法/模型路线"
            "（输入、架构、训练策略或计算设置）与关键定量结果，避免泛泛而谈；\n"
                "   2) related_work（≤100 字，1~2 句）：与团队研究方向的具体关系（对应哪个体系/方法），没有直接关系就直说；\n"
                "   3) implication（≤80 字）：一个可直接借鉴的点（数据、模拟条件或验证方式）。\n\n"
